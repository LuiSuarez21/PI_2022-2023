...
//Lê o ficheiro Ficheiro3.txt, guarda os valores na struct e retorna o numero de linhas preenchidas;
int Ler_Fich3(struct Utilizacao u1[]) {

    FILE* nomeFicheiro2 = fopen("ficheiro3.txt", "r");
    int f = 0;
    ...
    while (!feof(nomeFicheiro2))
    {
        fscanf(nomeFicheiro2, "%d %d %d %d %d %s\n", &u1[f].ordem, &u1[f].NIF, &u1[f].tempoInicio, &u1[f].tempoFim, &u1[f].autonomiaInicial, u1[f].codigo);
        f++;
    }
    fclose(nomeFicheiro2);
